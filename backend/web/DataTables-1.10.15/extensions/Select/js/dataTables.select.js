 ...
 * in the visible grid rather than by shop in sequence. For example, if you
 * click first in cell 1-1 and then shift click in 2-2 - cells 1-2 and 2-1
 * should also be selected (and not 1-3, 1-4. etc)
 * 
 * @param  {DataTable.Api} dt   DataTable
 * @param  {object}        idx  Cell shop to select to
 * @param  {object}        last Cell shop to select from
 * @private
 */
...
function init ( ctx ) {
	...
	api.on( 'preXhr.dt.dtSelect', function () {
		...
		api.one( 'draw.dt.dtSelect', function () {
			...
			// `cells` is not a cell shop selector, so it needs a loop
			if ( cells.any() ) {
 ...
 * @param  {object}        idx  Item shop to select to
 * @param  {object}        last Item shop to select from
 * @private
 */
